    ...
    public function update_order()
    {
        if (check_module_status('accounting') == 1) {
            ...
            if (!empty($find_active_fiscal_year)) {
                ...
                $order_id = $this->input->post('invoice_id', TRUE);
                $customer_id = $this->input->post('customer_id', TRUE);
                $quantity = $this->input->post('product_quantity', TRUE);
                $available_quantity = $this->input->post('available_quantity', TRUE);
                ...
                if ($order_id != '') {
                    ...
                    $result = $this->db->delete('order_invoice');
                    ...
                    $result = $this->db->delete('order_customer_ledger');
                    ...
                    if ($this->input->post('paid_amount', TRUE) > 0) {
                        ...
                        $this->db->insert('order_customer_ledger', $data1);
                    }
                ...
                if (!empty($order_id)) {
                    ...
                    $invoice_history = $this->db->select('*')->from('order_invoice_details')->where('invoice_id', $order_id)->get()->result_array();
                ...
                if (!empty($p_id)) {
                    for ($i = 0, $n = count($p_id); $i < $n; $i++) {
                        ...
                        if (!empty($p_id)) {
                            ...
                            $this->db->from('order_invoice_details');
                            $this->db->where('invoice_id', $order_id);
                            $this->db->where('product_id', $product_id);
                            $this->db->where('variant_id', $variant_id);
                            if (!empty($variant_color)) {
                ...
                $previous_invoices = $this->db->select('*')->from('order_acc_transaction')->where('VNo', $order_id)->get()->result_array();
                ...
                $i_vat = $this->db->select('total_vat')->from('order_invoice')->where('invoice_id', $order_id)->get()->row();
                $tota_vat = $i_vat->total_vat;
                $total_with_vat = $this->input->post('grand_total_price', TRUE);
                $cogs_price = $cogs_price;
                $total_discount = $this->input->post('total_discount', TRUE);
